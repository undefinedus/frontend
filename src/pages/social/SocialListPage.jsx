...
import {
  getSocialList,
  patchFollow,
  getSocialInfo, // 최신 프로필 정보 가져오는 API 추가
...
const SocialListPage = () => {
  ...
  const [newSocialProfile, setNewSocialProfile] = useState(socialProfile); // 최신 프로필 상태
  ...
  useEffect(() => {
    fetchSocialList(lastId);
    console.log(`Fetch완료 search ${search} activeTab ${activeTab}`);
  }, [search, activeTab]);

  // 최신 프로필 API 호출
  const fetchMySocialInfo = async () => {
    try {
      const response = await getSocialInfo();
      console.log("***** 유저 최신 프로필 정보 :", response.data);
      setNewSocialProfile(response.data); // 최신 프로필 업데이트
    } catch (error) {
  ...
  // 소셜 목록 API 호출
  const fetchSocialList = async (lastId = null) => {
    try {
      setIsLoading(true);
      ...
      console.log("***** 유저 팔로워, 팔로잉 목록 :", response);
      console.log("***** activeTab  :", activeTab);
      if (lastId) {
        setSocialList((prevList) => ({
          ...prevList,
          content: [...prevList.content, ...response.content],
  ...
  useEffect(() => {
    const fetchSocialProfile = async () => {
      try {
        const updatedProfile = await getSocialInfo(); // 서버에서 최신 프로필 가져오기
        setNewSocialProfile(updatedProfile.data); // 최신 데이터로 상태 업데이트
      } catch (error) {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      if (
        ...
        fetchSocialList(socialList.lastId);
      }
    };

    window.addEventListener("scroll", handleScroll);
    return () => window.removeEventListener("scroll", handleScroll);
  }, [socialList.hasNext, isLoading, page]);
